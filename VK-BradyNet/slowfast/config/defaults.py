...
# -----------------------------------------------------------------------------
# RGB + Keypoint feature fusion config
# -----------------------------------------------------------------------------
_C.FUSION = CfgNode()
# Enable RGB-keypoint feature-level fusion model and dataloader keypoint loading.
_C.FUSION.ENABLE = False
# Keypoint file root. If CSV gives relative keypoint paths, this prefix will be used.
_C.FUSION.KEYPOINT_ROOT = ""
# Number of frames used by keypoint branch. Usually equals DATA.NUM_FRAMES.
_C.FUSION.KEYPOINT_NUM_FRAMES = 16
# Number of hand landmarks. MediaPipe Hands uses 21.
_C.FUSION.NUM_KEYPOINTS = 21
# Number of values per keypoint. Recommended 3: x_norm, y_norm, z_norm.
_C.FUSION.KEYPOINT_DIM = 3
# Keypoint encoder type: "transformer" or "bilstm".
_C.FUSION.ENCODER = "transformer"
_C.FUSION.EMBED_DIM = 128
_C.FUSION.NUM_LAYERS = 2
_C.FUSION.NUM_HEADS = 4
_C.FUSION.DROPOUT = 0.3
_C.FUSION.FUSION_DROPOUT = 0.5
# If True, freeze RGB MViT backbone and only train keypoint encoder + fusion head.
_C.FUSION.FREEZE_RGB = False
# Missing keypoint npy behavior: "zeros" or "error".
_C.FUSION.MISSING_KEYPOINT = "zeros"
# Use motion-speed guided temporal attention in the keypoint encoder. This is designed
# for PD severity tasks where slower hand movement is clinically meaningful.
_C.FUSION.SPEED_ATTENTION = True
# Alpha used by sigmoid(-alpha * speed) to emphasize slow-motion frames.
_C.FUSION.SPEED_ATTENTION_ALPHA = 10.0
# Use a learnable gate to control how much keypoint information enters the fusion head.
_C.FUSION.GATED_FUSION = True
# Optional scalar weight applied to keypoint features before fusion.
_C.FUSION.KEYPOINT_FUSION_SCALE = 1.0
