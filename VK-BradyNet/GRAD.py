...
功能：
1. 从测试集 score_0 / score_1 / score_2 每类随机选择 20 个样本，总计最多 60 个。
...
OUTPUT_DIR = (
    PROJECT_ROOT
    / "outputs"
    / "HM_gradcam_keypoint_fused_random60_original_video_final"
)
...
def score_class_name(score_idx):
    return f"HM_score_{int(score_idx)}"


def score_short_name(score_idx):
    return f"HM{int(score_idx)}"
...
def select_random_indices_by_score(keypoint_provider):
    rng = random.Random(RANDOM_SEED)

    pools = {int(c): [] for c in TARGET_SCORES}
    ...
    for score in TARGET_SCORES:
        candidates = pools[int(score)]
        rng.shuffle(candidates)

        n = min(SAMPLES_PER_SCORE, len(candidates))
        selected[int(score)] = candidates[:n]

        for idx in selected[int(score)]:
            selected_set.add(int(idx))

    selected_info = {
        "random_seed": int(RANDOM_SEED),
        "target_scores": [int(c) for c in TARGET_SCORES],
        "samples_per_score": int(SAMPLES_PER_SCORE),
        "num_selected_total": int(len(selected_set)),
        "score_counts_available": {
            str(score): int(len(pools[int(score)])) for score in TARGET_SCORES
        },
        "selected_indices_by_score": {
            str(score): [int(x) for x in selected[int(score)]]
            for score in TARGET_SCORES
        },
    }
...
class VideoGradCAM:
    ...
    def compute_cam(self, target_score, inputs, keypoints):
        ...
        score = logits[0, int(target_score)]
...
def compute_input_gradient_heatmap(model, inputs, keypoints, target_score):
...
def compute_keypoint_attribution(model, inputs, keypoints, target_score):
...
def main():
    ...
    print("Random balanced sampling: score 0/1/2 × 20 samples")
    ...
    print(f"Each score:                {SAMPLES_PER_SCORE}")
    ...
    selected_by_score, selected_set, selected_info = select_random_indices_by_score(
        keypoint_provider
    )
    ...
    for score in TARGET_SCORES:
        print(f"  score_{score}: {selected_by_score[int(score)]}")

    expected_total = len(selected_set)

    if expected_total == 0:
        raise RuntimeError(
            "No selected samples. Please check test.csv labels or npy/test score folders."
        )
    ...
    generated_counts = {int(c): 0 for c in TARGET_SCORES}
    ...
    for batch_idx, batch in enumerate(test_loader):
        ...
        true_score = int(labels_np[0])

        if true_score not in TARGET_SCORES:
            global_sample_start += batch_size
            continue
        ...
        pred_score = int(torch.argmax(probs_pred[0]).item())
        pred_prob = float(probs_pred[0, pred_score].detach().cpu().item())
        ...
        current_score_count = generated_counts[int(true_score)]
        score_dir = ensure_dir(OUTPUT_DIR / f"score_{true_score}")
        sample_dir = ensure_dir(score_dir / f"sample_{current_score_count:03d}")
        ...
        label_text = (
            f"T:{score_short_name(true_score)} | "
            f"P:{score_short_name(pred_score)} | "
            f"p={pred_prob:.3f}"
        )
        ...
        save_temporal_saliency_plot(
            ...
            title=(
                ...
                f"({score_class_name(true_score)} → {score_class_name(pred_score)})"
            ),
        )
        ...
        save_fused_panel(
            ...
            title=(
                ...
                f"True={score_short_name(true_score)}, "
                f"Pred={score_short_name(pred_score)}, "
                f"p={pred_prob:.3f}"
            ),
        )
        ...
        prediction_info = {
            ...
            "true_score_name": score_class_name(true_score),
            "pred_score": int(pred_score),
            "pred_score_name": score_class_name(pred_score),
            ...
            "score_sample_number": int(current_score_count),
        ...
        debug_stats["selected_true_score"] = int(true_score)
    ...
    for score in TARGET_SCORES:
        print(f"  score_{score}: {generated_counts[int(score)]}")
    print("=" * 80)
