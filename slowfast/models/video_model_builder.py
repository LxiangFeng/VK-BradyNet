...
class KeypointTemporalEncoder(nn.Module):
    """
    Encode a hand-keypoint sequence into a compact video-level feature.

    Expected enhanced keypoint format when cfg.FUSION.KEYPOINT_DIM == 10:
        [x, y, z, dx, dy, dz, speed, accel, dist_to_wrist, confidence]

    The encoder keeps backward compatibility with 3-D and 6-D keypoints, but if
    a speed channel is available it uses a clinically motivated temporal attention
    mechanism: slower frames receive higher attention weights because higher PD
    severity is usually associated with bradykinesia / reduced hand motion speed.
    """

    def __init__(self, cfg):
        super().__init__()
        self.num_keypoints = int(cfg.FUSION.NUM_KEYPOINTS)
        self.keypoint_dim = int(cfg.FUSION.KEYPOINT_DIM)
        self.input_dim = self.num_keypoints * self.keypoint_dim
        self.embed_dim = int(cfg.FUSION.EMBED_DIM)
        self.encoder_type = cfg.FUSION.ENCODER.lower()
        self.use_speed_attention = bool(getattr(cfg.FUSION, "SPEED_ATTENTION", True))
        self.speed_attention_alpha = float(getattr(cfg.FUSION, "SPEED_ATTENTION_ALPHA", 10.0))

        self.input_proj = nn.Linear(self.input_dim, self.embed_dim)
        self.dropout = nn.Dropout(float(cfg.FUSION.DROPOUT))
        ...
        if self.encoder_type == "bilstm":
            ...
            self.encoder = nn.LSTM(
                ...
                hidden_size=hidden,
                num_layers=int(cfg.FUSION.NUM_LAYERS),
                batch_first=True,
                bidirectional=True,
                dropout=float(cfg.FUSION.DROPOUT) if int(cfg.FUSION.NUM_LAYERS) > 1 else 0.0,
            )
            self.out_dim = hidden * 2
        else:
            layer = nn.TransformerEncoderLayer(
                d_model=self.embed_dim,
                nhead=int(cfg.FUSION.NUM_HEADS),
            ...
            self.encoder = nn.TransformerEncoder(layer, num_layers=int(cfg.FUSION.NUM_LAYERS))
            self.out_dim = self.embed_dim

        self.norm = nn.LayerNorm(self.out_dim)

    def _reshape_keypoints(self, keypoints):
        """Return keypoints as [B, T, K, C] and flattened [B, T, K*C]."""
        if keypoints is None:
            raise ValueError("MViTFusion requires keypoints tensor, but got None.")
        keypoints = keypoints.float()
        if keypoints.ndim == 4:
            kpts_4d = keypoints
        elif keypoints.ndim == 3:
            b, t, d = keypoints.shape
            expected = self.num_keypoints * self.keypoint_dim
            if d < expected:
                pad = torch.zeros(
                    b, t, expected - d,
                ...
                keypoints = keypoints[:, :, :expected]
            kpts_4d = keypoints.reshape(b, t, self.num_keypoints, self.keypoint_dim)
        else:
            raise ValueError("Unsupported keypoints shape: {}".format(tuple(keypoints.shape)))

        # Defensive padding/cropping for K or C mismatches.
        b, t, k, c = kpts_4d.shape
        if k < self.num_keypoints or c < self.keypoint_dim:
            padded = torch.zeros(
                b, t, self.num_keypoints, self.keypoint_dim,
                dtype=kpts_4d.dtype,
                device=kpts_4d.device,
            )
            padded[:, :, :min(k, self.num_keypoints), :min(c, self.keypoint_dim)] =                 kpts_4d[:, :, :min(k, self.num_keypoints), :min(c, self.keypoint_dim)]
            kpts_4d = padded
        else:
            kpts_4d = kpts_4d[:, :, :self.num_keypoints, :self.keypoint_dim]

        flat = kpts_4d.flatten(2)
        return kpts_4d, flat

    def _speed_attention_weights(self, kpts_4d):
        """
        Build frame-level weights [B, T, 1] from the speed channel.
        If no speed channel exists, returns uniform weights.
        """
        b, t, _, c = kpts_4d.shape
        if not self.use_speed_attention or c < 7:
            return torch.ones(b, t, 1, dtype=kpts_4d.dtype, device=kpts_4d.device)

        # Channel 6 is per-keypoint speed. Average over 21 landmarks.
        speed = kpts_4d[..., 6].clamp(min=0.0).mean(dim=2, keepdim=False)  # [B, T]

        # Confidence channel 9 if available. If a frame was not detected, do not
        # treat zero speed as clinically slow; down-weight it instead.
        if c >= 10:
            conf = kpts_4d[..., 9].mean(dim=2).clamp(0.0, 1.0)  # [B, T]
        else:
            conf = torch.ones_like(speed)

        # Slow motion -> larger raw score. Fast motion -> smaller raw score.
        # Add 0.05 to keep weights non-zero for valid faster frames.
        raw = torch.sigmoid(-self.speed_attention_alpha * speed) * conf + 0.05 * conf
        denom = raw.sum(dim=1, keepdim=True).clamp(min=1e-6)
        weights = raw / denom * float(t)
        return weights.unsqueeze(-1)  # [B, T, 1]

    def forward(self, keypoints):
        # keypoints: [B, T, K*C] or [B, T, K, C]
        kpts_4d, flat = self._reshape_keypoints(keypoints)
        attn_w = self._speed_attention_weights(kpts_4d)

        x = self.input_proj(flat)
        x = self.dropout(x)

        # Apply speed-aware weighting before temporal encoding.
        x = x * attn_w
        ...
        # Weighted temporal pooling. This emphasizes clinically informative slow
        # movement segments while preserving sequence-level representation.
        denom = attn_w.sum(dim=1).clamp(min=1e-6)
        x = (x * attn_w).sum(dim=1) / denom
        x = self.norm(x)
        return x


@MODEL_REGISTRY.register()
class MViTFusion(MViT):
    """
    RGB + keypoint feature-level fusion model.

    RGB branch: original MViTv2-S backbone.
    Keypoint branch: lightweight Transformer/BiLSTM over [T, 21*3].
    Fusion: concat(rgb_feature, keypoint_feature) -> classifier.
    """

    def __init__(self, cfg):
        super().__init__(cfg)
        assert not self.enable_detection, "MViTFusion supports classification only."

        # The original TransformerBasicHead projection stores RGB feature dim.
        rgb_dim = self.head.projection.in_features
        self.rgb_feature_dim = rgb_dim

        self.keypoint_encoder = KeypointTemporalEncoder(cfg)
        kpt_dim = self.keypoint_encoder.out_dim
        self.keypoint_feature_dim = kpt_dim
        self.use_gated_fusion = bool(getattr(cfg.FUSION, "GATED_FUSION", True))
        self.keypoint_fusion_scale = float(getattr(cfg.FUSION, "KEYPOINT_FUSION_SCALE", 1.0))

        if cfg.FUSION.FREEZE_RGB:
            for name, p in self.named_parameters():
                p.requires_grad = False
            for p in self.keypoint_encoder.parameters():
                p.requires_grad = True

        # Gate learns how much keypoint information should be injected into the
        # classifier for each sample. This prevents a weak or noisy keypoint
        # branch from overwhelming the RGB branch.
        if self.use_gated_fusion:
            self.kpt_gate = nn.Sequential(
                nn.Linear(rgb_dim + kpt_dim, kpt_dim),
                nn.Sigmoid(),
            )
        else:
            self.kpt_gate = None
        ...
        self.head = nn.Sequential(
            nn.Dropout(float(cfg.FUSION.FUSION_DROPOUT)),
            nn.Linear(rgb_dim + kpt_dim, rgb_dim),
            nn.GELU(),
            nn.Dropout(float(cfg.MODEL.DROPOUT_RATE)),
            nn.Linear(rgb_dim, cfg.MODEL.NUM_CLASSES),
        )

    def _forward_rgb_features(self, x, bboxes=None):
        x = x[0]
        x, bcthw = self.patch_embed(x)
        bcthw = list(bcthw)
        if len(bcthw) == 4:
        ...
        thw = [T, H, W]
        if self.enable_rev:
            raise NotImplementedError("MViTFusion does not support reversible MViT.")
    ...
    def forward(self, x, keypoints=None, bboxes=None, return_attn=False):
        rgb_feat = self._forward_rgb_features(x, bboxes=bboxes)
        kpt_feat = self.keypoint_encoder(keypoints)

        if self.kpt_gate is not None:
            gate = self.kpt_gate(torch.cat([rgb_feat, kpt_feat], dim=1))
            kpt_feat = kpt_feat * gate

        kpt_feat = kpt_feat * self.keypoint_fusion_scale
        fused = torch.cat([rgb_feat, kpt_feat], dim=1)
        out = self.head(fused)
        return out
